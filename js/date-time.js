class DateTime {
  ...
  _getDayOrdinal(day) {
    return (
      day +
      (this._Language._language == "en_us"
        ? day > 0
          ? ["th", "st", "nd", "rd"][
  ...
  _setTime() {
    ...
    if (hour >= 6 && hour < 12) {
      ...
      greeterSuffix = this._Language._getTranslatedItem(
        "afternoon",
        "Afternoon"
      );
    } else {
    ...
    if (this._twentyFourMode === true) {
      ...
      hour =
        hour === 0
          ? 12
          : hour > 12
          ? this._appendZero(hour - 12)
          : this._appendZero(hour);
    ...
    this._greeterDate.innerText =
      `${this._getDayOrdinal(
        this._appendZero(date.getDate())
      )} ${this._Language._getTranslatedItem("of", "of")} ` +
  ...
  _getSidebarDateLocalized(date) {
    ...
    if (this._Language._language == "pt_br") {
      return (
        `${this._daysArr[date.getDay()]}, ${this._appendZero(
          date.getDate()
        )} ${this._Language._getTranslatedItem("of", "de")} ` +
        `${
          this._monthsArr[date.getMonth()]
        } ${this._Language._getTranslatedItem(
          "of",
          "de"
        )} ${date.getFullYear()}`
      );
  ...
  _updateClockMode() {
    ...
    this._localStorage.setItem(
      "twentyFourMode",
      JSON.stringify(this._twentyFourMode)
    );
    this._startClock();
  }

  _clockClickEvent() {
    this._greeterClock.addEventListener("click", () => {
      console.log("toggle 24-hour clock mode");
      this._updateClockMode();
    });
    this._sidebarClock.addEventListener("click", () => {
      console.log("toggle 24-hour clock mode");
      this._updateClockMode();
    });
  }
  ...
  _init() {
    this._twentyFourMode =
      JSON.parse(this._localStorage.getItem("twentyFourMode")) || false;
    this._startClock();
