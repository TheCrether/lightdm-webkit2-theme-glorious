class Power {
  constructor() {
    ...
    this._powerList = document.querySelector(
      "#power-list.sidebar-settings-item-list"
    );
    this._powerObject = [];
    this._init();
  }
    ...
    this._powerObject = [
      {
        ...
        message: language._getPowerTranslate(
          "shutdown",
          "Shutting down...",
          "message"
        ),
  ...
  _createPowerList() {
    for (let i = 0; i < this._powerObject.length; i++) {
      ...
      this._powerItemOnClickEvent(
        powerItemButton,
        this._powerObject[parseInt(i, 10)]
      );
