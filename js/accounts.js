class Accounts {
  constructor() {
    ...
    this._accountsList = document.querySelector(
      "#accounts-list.sidebar-settings-item-list"
    );
  ...
  _userItemCreateClickEvent(userObject) {
    userObject.button.addEventListener("click", () => {
      ...
      this._setUserImageProfile(
        userObject.profileImage,
        userObject.profileImageFallBack
      );
  ...
  _setAccountDefaultOnStartUpFallback() {
    ...
    this._defaultUserDisplayName =
      this._usersObject[parseInt(0, 10)].display_name;
  ...
  _setAccountDefaultOnStartUp() {
    let defaultAccountItem = document.querySelector(
      `#button-accounts-item-${this._defaultUser}`
    );
    if (!defaultAccountItem) {
      // If the should've been default user does not exist
      defaultAccountItem = document.querySelector(
        `#button-accounts-item-${this._usersObject[parseInt(0, 10)].username}`
      );
  ...
  _updateProfileVariablesOnStartUp() {
    ...
    this._defaultUserProfileImageFallback =
      this._localStorage.getItem("defaultUserProfileImageFallback") ||
